...
class Buttons extends React.Component {
  ...
  timerTaskFunction = () => {
    ...
    //计算登陆超时超时
    if (this.loginTime > this.state.againTryConTimes) {
  ...
  // *点击取消的时候触发此事件，
  resetLoginTime = () => {
    window.clearLoginAndRegisterTimer&&window.clearLoginAndRegisterTimer()
    return true
    console.log('停止');
    this.loginTime = 0;
    this.loginTime2 = 0;
  ...
  cmtLoginRegister = () => {
    ...
    if (res == 0) {
      ...
      if (this.loginTime2 > this.state.againTryConTimes) {
  ...
  componentDidMount() {
    ...
    */
    this.mso = GetCmtWsClientHelper();
    let cmtStatus = window.cmtLoginStatus
    let cmtLoading = window.localStorage.getItem('zhinengjianceloading')
    if (cmtLoading == 'login-success') {
    ...
    cmtEvents.on('onEventRegistered',(parameter) => {
      ...
      if (code == 200) {
        ...
        cmtEvents.emit('setMso',this.mso)
    ...
    return (
      <div className="btns-group">
        <div className="common-btns">
          ...
          {/* <Button onClick={this.qchai}>强拆</Button> */}
