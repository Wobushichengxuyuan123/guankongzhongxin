...
import Time from './time/timezhou';
...
export default class Main extends React.Component {
            ...
            success: function (xmlDoc) {
                ...
                if ("OK" === $(xmlDoc).find("responseStatusStrg").eq(0).text()) {
                    ...
                    for (var i = 0; i < iLength; i++) {
                        ...
                        let a = (starttime.replace("T", " ")).replace("Z", "")
                        let b = (endTime.replace("T", " ")).replace("Z", "")
                        console.log(a, b);
                        list.push({
                            entrance_time: moment(a).valueOf(),
                            leave_time: moment(b).valueOf()
                        })
                    }
                    that.setState({
                        timeList: list
                    }, () => {
                        console.log(that.state.timeList);
                    })
    ...
    render() {
        const { startTime, endTime, lWidth, editWidth, treeData, timeList } = this.state
        const Loop = (data) => {
            return data.map((item, i) => {
                if (item.children != null) {
                    return <TreeNode icon={<Icon type="bars" />} title={<span >{item.name}</span>} key={item.id + i}>
                        {Loop(item.children)}
                    </TreeNode>
                } else {
                    return <TreeNode icon={<Icon type="video-camera" />} title={<span onClick={this.seticneelid.bind(this, item)}>{item.name}</span>} key={item.id + i} />
                }
            })
        }
        ...
        return (<div className="r_flex contain">
            <div style={{ width: `calc(100% - 59px)`, display: 'flex', flexDirection: 'row', overflowY: 'scroll', }}>
                <div style={{
                    ...
                    <div className="title">视频回放</div>
                    <Tree showIcon >{Loop(treeData)} </Tree>
                </div>
                <div className="demo-blend" style={{ width: `${editWidth}%` }}>
                    <div className="videowrap" >
                        ...
                        <Time className='time' timelist={timeList} startTime={startTime} endTime={endTime} iWndIndex={this.state.iWndIndex} parent={this} onRef={ref => this.child = ref} />
                    </div>
                    {mainBtn()}
                </div>
            </div>
