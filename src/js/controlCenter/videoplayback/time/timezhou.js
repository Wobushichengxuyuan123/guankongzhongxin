...
class Main extends React.Component {
  ...
  componentWillMount() {
    // if (this.props.startTime && this.props.timelist)
    // this.setState({
    //   iWndIndex: this.props.iWndIndex,
    //   timeList:this.props.timelist
    // })
    // this.getSearchDetails(this.props.startTime, this.props.timelist)
  }
  componentWillReceiveProps(nextProps) {
    if (nextProps.startTime && nextProps.timelist.length>0) {
      this.getSearchDetails(nextProps.startTime, nextProps.timelist)
    }
  }
  // 组件销毁
  ...
  getSearchDetails(start, last) {
    let list = last
    let showTime = list[0].entrance_time ? moment(list[0].entrance_time * 1).format('HH:mm:ss') : "00:00:00"
    ...
    this.setState({
      ...
      showTime,
      isShow: true,
      showTimeGetTime: list[0].entrance_time * 1,
      domEndTime: list[0].leave_time * 1
    })
  }
