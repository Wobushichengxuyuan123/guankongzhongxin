...
class Main extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isLoding: true,
      resNum: 0,
      alarmComfrom: this.props.alarmComfrom,
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.alarmComfrom !== state.alarmComfrom) {
      return {
        alarmComfrom: props.alarmComfrom,//把父组件最新的props.type重新赋值到 子组件state.type 
      }
    }
    return null
  }
  componentDidUpdate(prevProps, prevState) {
    if (prevState.alarmComfrom != this.state.alarmComfrom) {
      this.getAlarmInfo(this.state.type, this.state.alarm_type);
    }
  }
  ...
  getAlarmInfo(type, alarm_type, name, startValue, endValue) {
    ...
    let param = "&type=" + type + "&alarm_type=" + alarm_type + "&date=" + this.state.date;
    if (alarm_type === 2) {
      param += "&name=" + name + "&start_time=" + (startValue ? startValue : "") + "&end_time=" + (endValue ? endValue : "");
    }
    fetch(window.BASICS_SYSTEM + "/pubAlarmSearch/alarmInfo?=" + param + "&alarmType=" + this.state.alarmComfrom)
      .then(r => r.json())
      .then(b => {
        if (b.data) {
  ...
  getAlarmInfoDetails(type, alarm_type, id, pageNo, pageSize) {
    ...
    let param = "&type=" + type + "&alarm_type=" + alarm_type + "&id=" + id + '&pageNo=' + pageNo + '&pageSize=' + pageSize + "&date=" + this.state.date;
    if (alarm_type === 2) {
    ...
    fetch(window.BASICS_SYSTEM + "/pubAlarmSearch/alarmInfoDetails?alarmType=" + this.state.alarmComfrom + param)
      .then(r => r.json())
      .then(b => {
        if (b.data) {
          this.setState({
  ...
  changeSearch(e) {
    if (e.trim() == "") {
      var param = "&type=" + this.state.type + "&start_time=" + this.state.startValue + "&end_time=" + this.state.endValue + "&date=" + this.state.date;
    } else if (e.trim() != "") {
    ...
    fetch(window.BASICS_SYSTEM + "/pubAlarmSearch/alarmInfoDetailsAndCount?=" + param + '&alarmType=' + this.state.alarmComfrom)
      .then(r => r.json())
      .then(b => {
        if (b.data) {
          this.setState({ alarmInfo: b.data });
