...
class Main extends React.Component {
  ...
  dwHander(data) {
    let param = {
      ...
      alarmTitle: data.alarm_title,
      alarm_categories_id: data.alarm_categories_id
    }
    if (window.PushData) {
  ...
  render() {
    ...
    return (<div className="alarmItem" onClick={this.dwHander.bind(this, this.props.data)}>
      ...
      {this.props.parent.state.historySearch ?
        <div className="alarmTime">报警时间：{new Date(this.props.data.alarm_time).Format(format)} </div>
        :
        <div className="alarmTime">
          时间： {new Date(this.props.data.alarm_time).Format(format)}
          时长：{window.TimeDifference(this.state.serverDate - new Date(this.props.data.alarm_time).getTime())}
        </div>}
