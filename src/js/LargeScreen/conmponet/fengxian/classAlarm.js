...
class ClassAlarm extends React.Component {
  ...
  getAlarmInfo(type, alarm_type, name, startValue, endValue) {
    ...
    fetch(window.BASICS_SYSTEM + "/pubAlarmSearch/alarmInfo?="  + param + '&alarmType=F'+ '&date=' + this.state.data)
      .then(r => r.json())
      .then(b => {
        if (b.data) {
  ...
  getAlarmInfoDetails(type, alarm_type, id, pageNo, pageSize) {
    ...
    fetch(window.BASICS_SYSTEM + "/pubAlarmSearch/alarmInfoDetails" + param + '&alarmType=F')
      .then(r => r.json())
      .then(b => {
        if (b.data) {
          this.setState({
