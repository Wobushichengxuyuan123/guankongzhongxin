...
bxmap.FlyCesium = {
  ...
  clearFly3DPaths: function () {
    ...
    for (
      var index = 0;
      index < viewer.entities._entities._array.length;
      index++
    ) {
      var utc = Cesium.JulianDate.fromDat;
      if (!viewer.entities._entities._array[index].Ttype) {
        this.cesiumViewer.entities.remove(
          viewer.entities._entities._array[index]
        );
      }
    }
    //清空绘制飞行路线
  ...
  showFly3DPaths: function (pathsData) {
    ...
    function executeFly3D() {
      if (pathsData && pathsData.geometry) {
        ...
        T.cesiumViewer.clock.multiplier = 0.01; //值越大，飞行越快
  ...
  // 默认路径飞行
  PresetsPathsFn: function (PresetsPaths) {
    var T = this;
    this.clearFly3DPaths();
    drawHelper.startDrawingPolyline({
      callback: function (positions) {
