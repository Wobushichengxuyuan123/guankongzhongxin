          ...
          var handleEditMarkerChanges = {
            dragHandlers: {
              ...
              onDragEnd: function (index, position) {
                ...
                editMarkers.getBillboard(this.index - 1).position =
    ...
    function _(drawHelper, options) {
      ...
      addIcon("circle", options.circleIcon, "画圆", function () {
        drawHelper.startDrawingCircle({
          showFly3DPathscallback: function (center, radius) {
            _self.executeListeners({
              name: "circleCreated",
              center: center,
              radius: radius,
      ...
      addIcon("clear", options.clearIcon, "清除图形", function () {
        ...
        for (var i = 0; i < length; ++i) {
          ...
          if (p) {
            if (p.hasOwnProperty("type") && p.type == "plot") {
              //线、面、矩形、圆形
              primitives.remove(p);
            } else {
              //点图标类型
