 ...
 */
      ...
      case 'polyline':
        ...
        handler.setInputAction(function (click) {
          ...
          if (pick && pick.id && pick.id.label) {
            ...
            tempEntities.push(point);
            if (tempLength > 1) {
              let pointline = drawObj.drawPolyline([tempPoints[tempPoints.length - 2], tempPoints[tempPoints.length - 1]]);
        ...
        handler.setInputAction(function (click) {
          ...
          if (cartesian) {
            ...
            if (tempLength < 3) {
              ...
              //闭合最后一条线
              let pointline = drawObj.drawPolyline([tempPoints[tempPoints.length - 1], tempPoints[0]]);
              tempEntities.push(pointline);
  ...
  drawPoint: function (position, config_) {
    ...
    return viewer.entities.add({
      ...
      position: position,
      point: {
        color: config.color ? new Cesium.Color.fromCssColorString(config.color) : Cesium.Color.SKYBLUE,
        pixelSize: 5,
        outlineColor: config.outlineColor ? new Cesium.Color.fromCssColorString(config.outlineColor) : Cesium.Color.YELLOW,
        outlineWidth: 2,
  ...
  drawPolygon: function (positions, config_) {
    ...
    return viewer.entities.add({
      ...
      polygon: {
        ...
        material: config.color ? new Cesium.Color(config.color[0], config.color[1], config.color[2], Number(config.opacity)) : new Cesium.Color.fromCssColorString("#FFD700").withAlpha(.2),
      },
    });
  },
  ...
  drawLine: function (data) {
    drawObj.clearDrawEntities()
    if (data.length > 1) {
      const notPositions = data.filter(item => !item.status).map(v => {
        const position = Cesium.Cartesian3.fromDegrees(Number(v.xCoordinate), Number(v.yCoordinate), Number(v.zCoordinate));
        drawObj.drawPoint(position);
        return position;
      });
      const positions = data.filter(item => item.status).map(v => {
        const position = Cesium.Cartesian3.fromDegrees(Number(v.xCoordinate), Number(v.yCoordinate), Number(v.zCoordinate));
        drawObj.drawPoint(position, {
          color: '#D2691E',
          outlineColor: '#FFEBCD'
      ...
      if (notPositions.length) {
        if (notPositions.length === data.length) {
          ...
          // 部分未巡检
          // 获取已巡检点位最后值
          const lastObj = data.filter(item => item.status)[positions.length - 1];
