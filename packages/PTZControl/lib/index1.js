(function (global, factory) {
	...
	function YunController(props) {
	  ...
	  var _useState9 = React.useState(props.address),
	      _useState10 = _slicedToArray(_useState9, 2),
	      address = _useState10[0],
	      setAddress = _useState10[1]; // 摄像头IP地址


	  var _useState11 = React.useState(false),
	      _useState12 = _slicedToArray(_useState11, 2);
	      _useState12[0];
	      _useState12[1]; // 是否发现设备


	  var _useState13 = React.useState([]),
	      _useState14 = _slicedToArray(_useState13, 2),
	      presetList = _useState14[0],
	      setPresetList = _useState14[1]; // 预置位列表


	  var _useState15 = React.useState('云台正在连接摄像头设备，请稍后...'),
	      _useState16 = _slicedToArray(_useState15, 2);
	      _useState16[0];
	      _useState16[1];

	  var _useState17 = React.useState(new WebSocket(props.serverIp)),
	  ...
	  var initWebSocketConnection = function initWebSocketConnection() {
	    ws.onopen = function () {
	      console.log('已建立WebSocket连接.', address); // sendRequest('startDiscovery', { address, ...props })

	      connectDevice(address, props.user, props.pass);
	    }; // ws响应
	  ...
	  var connectDevice = function connectDevice(address, user, pass) {
	    sendRequest('connect', {
	      address: address,
	      user: user,
	      pass: pass
